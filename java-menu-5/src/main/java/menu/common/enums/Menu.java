package menu.common.enums;

import camp.nextstep.edu.missionutils.Randoms;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashMap;
import java.util.List;
import java.util.Optional;
import java.util.stream.Collectors;

public enum Menu {
    JAPANESE(1, "일식", List.of("규동", "우동", "미소시루", "스시", "가츠동", "오니기리", "하이라이스", "라멘", "오코노미야끼")),
    KOREAN(2, "한식", List.of("김밥", "김치찌개", "쌈밥", "된장찌개", "비빔밥", "칼국수", "불고기", "떡볶이", "제육볶음")),
    CHINESE(3, "중식", List.of("깐풍기", "볶음면", "동파육", "짜장면", "짬뽕", "마파두부", "탕수육", "토마토 달걀볶음", "고추잡채")),
    ASIAN(4, "아시안", List.of("팟타이", "카오 팟", "나시고렝", "파인애플 볶음밥", "쌀국수", "똠얌꿍", "반미", "월남쌈", "분짜")),
    WESTERN(5, "양식", List.of("라자냐", "그라탱", "뇨끼", "끼슈", "프렌치 토스트", "바게트", "스파게티", "피자", "파니니"));

    private static final int MAX_CATEGORY_INDEX = 5;
    private static final Integer MAX_DUPLICATE_CATEGORY = 2;
    private final int index;
    private final String category;
    private final List<String> menu;

    Menu(int index, String category, List<String> menu) {
        this.index = index;
        this.category = category;
        this.menu = menu;
    }

    public static boolean notContains(String menuName) {
        return Arrays.stream(Menu.values())
                .flatMap(menu -> menu.getMenu().stream())
                .noneMatch(menu -> menu.equals(menuName));
    }

    public static List<Menu> getWeeklyCategories() {
        List<Integer> indices = pickWeeklyCategoryIndices();
        return indices.stream()
                .map(Menu::getMenuByIndex)
                .flatMap(Optional::stream)
                .collect(Collectors.toList());
    }

    private static List<Integer> pickWeeklyCategoryIndices() {
        HashMap<Integer, Integer> frequency = new HashMap<>();
        List<Integer> indices = new ArrayList<>();
        while (indices.size() < MAX_CATEGORY_INDEX) {
            int index = Randoms.pickNumberInRange(1, 5);
            if (frequency.getOrDefault(index, 0) < MAX_DUPLICATE_CATEGORY) {
                frequency.put(index, frequency.getOrDefault(index, 0) + 1);
                indices.add(index);
            }
        }
        return indices;
    }

    private static Optional<Menu> getMenuByIndex(int index) {
        return Arrays.stream(Menu.values())
                .filter(menu -> menu.getIndex() == index)
                .findFirst();
    }

    public String getCategory() {
        return category;
    }

    public int getIndex() {
        return index;
    }

    public List<String> getMenu() {
        return menu;
    }
}
